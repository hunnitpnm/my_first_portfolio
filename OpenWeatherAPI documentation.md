...
2. Использовать название населенного пункта или сочетание названия населенного пункта, кода страны. Для США дополнительно можно указать код штата
...
Формат выходных данных: **JSON** и **XML**. По умолчанию используется формат **JSON**. Для получения данных в формате **XML**, установите параметр `mode=xml` в запросе ([пример ответа в формате XML см. ниже](#пример-ответа-api-в-формате-xml)).
...
| `weather.description` | string  | Конкретный тип погоды           |
